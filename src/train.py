...
def load_data(data_path, use_woe=False):
    ...
    base_exclude = [
        ...
        'is_high_risk',         # Target
        'recency', 'frequency', 'monetary',  # Used to create target
        'refund_rate', 'refund_amount'       # Highly correlated with target
    ]
...
# ================================================================
# NEW: GRADIENT BOOSTING (XGBoost style with sklearn)
# ================================================================
...
def register_best_model():
    ...
    # Register the model (try different artifact names)
    for model_artifact in ['gradient_boosting_tuned', 'gradient_boosting_baseline', 
                           'random_forest_tuned', 'random_forest_baseline', 
                           'logistic_regression_model']:
        model_uri = f"runs:/{best_run_id}/{model_artifact}"
        try:
            mlflow.register_model(model_uri, "CreditRiskModel")
            print(f"Registered: CreditRiskModel")
            break
        except Exception:
            continue
...
def main():
    ...
    data_path = os.path.join(script_dir, "..", "data", "processed", "processed_data.csv")
    ...
    # Set MLflow tracking
    mlflow.set_tracking_uri(f"file:{os.path.join(model_dir, 'mlruns')}")
    ...
    rf_baseline, rf_baseline_metrics = train_random_forest_baseline(X_train, X_test, y_train, y_test)
    ...
    rf_tuned, rf_tuned_metrics = train_random_forest_tuned(X_train, X_test, y_train, y_test)
    ...
    gb_baseline, gb_baseline_metrics = train_gradient_boosting_baseline(X_train, X_test, y_train, y_test)
    ...
    gb_tuned, gb_tuned_metrics = train_gradient_boosting_tuned(X_train, X_test, y_train, y_test)
    ...
    best_recall = max(lr_metrics['recall'], rf_baseline_metrics['recall'], 
                      rf_tuned_metrics['recall'], gb_baseline_metrics['recall'],
                      gb_tuned_metrics['recall'])
    
    if gb_tuned_metrics['recall'] == best_recall:
        joblib.dump(gb_tuned, best_model_path)
        print(f" Saved: Gradient Boosting Tuned (Recall: {gb_tuned_metrics['recall']:.4f})")
    elif gb_baseline_metrics['recall'] == best_recall:
        joblib.dump(gb_baseline, best_model_path)
        print(f" Saved: Gradient Boosting Baseline (Recall: {gb_baseline_metrics['recall']:.4f})")
        ...
        print(f" Saved: Random Forest Baseline (Recall: {rf_baseline_metrics['recall']:.4f})")

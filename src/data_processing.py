...
class HighRiskTargetCreator(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        self.high_risk_cluster = int(profiles_df.loc[
            profiles_df['recency_mean'].idxmax(), 'cluster'
        ])
        self.cluster_profiles_ = profiles_df
        
        print("\n--- Target Variable Cluster Breakdown ---")
        print(profiles_df[['cluster', 'size', 'recency_mean', 'frequency_mean', 'monetary_mean']].to_string(index=False))
        print(f"High-Risk Cluster: {self.high_risk_cluster} (Highest recency: {profiles_df.loc[profiles_df['cluster'] == self.high_risk_cluster, 'recency_mean'].values[0]:.1f} days)")
        return self
    ...
    def transform(self, X, y=None):
        ...
        X_scaled = self.internal_scaler.transform(rfm[['recency', 'frequency_log', 'monetary_log']])
        ...
        output['is_high_risk'] = (predicted_clusters == self.high_risk_cluster).astype(int)
        return output
...
class WoEIVCalculator:
    ...
    def calculate_woe_iv(self, df, feature, binned_series):
        ...
        grouped['non_event_pct'] = (grouped['non_events'] + epsilon) / (total_non_events + epsilon)
        
        grouped['woe'] = np.log(grouped['event_pct'] / grouped['non_event_pct'])
        grouped['woe'] = grouped['woe'].clip(-5, 5)
        
        grouped['iv_component'] = (grouped['event_pct'] - grouped['non_event_pct']) * grouped['woe']
    ...
    def fit(self, df):
        ...
        print(f"Features to KEEP ({len(keep_features)}): {', '.join(keep_features['Feature'].tolist())}")
        print(f"Features to DROP ({len(drop_features)}): {', '.join(drop_features['Feature'].tolist())}")
        ...
        for feature, woe_map in self.woe_maps.items():
            if feature in df.columns:
                ...
                if edges is not None:
                    ...
                    df_woe[feature + '_woe'] = df[feature].astype(str).map(woe_map).fillna(0)
...
class FeaturePipeline(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, create_target=False, apply_woe=False, n_clusters=3, random_state=42):
        ...
        self.numerical_cols = [
            ...
            "refund_rate", "refund_amount", "weekend_ratio", "business_hour_ratio",
            "transaction_hour_std", "unique_productcategory", "unique_channelid", "unique_providerid"
        ]
    ...
    def fit(self, df, y=None):
        ...
        if self.create_target:
            ...
            rfm_data_with_id = raw_features[['CustomerId', 'recency', 'frequency', 'monetary']].copy()
        ...
        if self.apply_woe and self.create_target and self.target_creator is not None:
            ...
            woe_features_df = raw_features[self.numerical_cols + self.categorical_cols].copy()
    ...
    def transform(self, df):
        ...
        processed_df[self.numerical_cols] = self.imputer.transform(processed_df[self.numerical_cols])
        processed_df[self.numerical_cols] = self.scaler.transform(processed_df[self.numerical_cols])
        ...
        encoded_outputs = pd.DataFrame(encoded_array, columns=encoded_cols_clean, index=processed_df.index)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("TASK 4: PROXY TARGET WITH WoE/IV ANALYSIS")
    print("=" * 60)
    ...
    # Safe call to get_iv_scores
    try:
        iv_scores = pipeline.get_iv_scores()
        if iv_scores is not None:
            iv_path = os.path.join(model_dir, "iv_scores.csv")
            iv_scores.to_csv(iv_path, index=False)
            print(f"Saved IV scores: {iv_path}")
    except Exception as e:
        print(f"Note: IV scores not available: {e}")
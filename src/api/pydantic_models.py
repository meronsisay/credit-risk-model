...
class CustomerFeatures(BaseModel):
    ...
    weekend_ratio: float = Field(
        ..., ge=0, le=1, description="Weekend transaction ratio"
    )
    business_hour_ratio: float = Field(
        ..., ge=0, le=1, description="Business hour ratio"
    )
    transaction_hour_std: float = Field(
        ..., ge=0, description="Hour standard deviation"
    )
    unique_productcategory: int = Field(
        ..., ge=1, description="Unique product categories"
    )
...
class PredictionResponse(BaseModel):
    ...
    risk_probability: float = Field(
        ..., ge=0, le=1, description="Probability of being high-risk"
    )
